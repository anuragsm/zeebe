...
public final class BpmnUserTaskBehavior {
  ...
  private Either<Failure, PersistedForm> findFormByIdInSameDeployment(
    ...
    return formState
        .findFormByIdAndDeploymentKey(wrapString(formId), deploymentKey, context.getTenantId())
                    ...
                    new Failure(
                        String.format(
                            "Expected to find a form with id '%s' in deployment %s, but not found."
                                + " To resolve the incident please deploy a form with the same id together with the process.",
